...
mv t0, sp
addi t0, t0, 12
sw t1, 0(t0)     # initialise x to 9
p_1:             # "par" does not produce code
p_2:             # "par" does not produce code
p_3:
...
addi t0, t0, 12
mv t1, t0
sw t1, 12(fp)    # store a reference to x (sp + 12) to the inout parameter (fp + 12)
mv t0, sp
addi t0, t0, 16
mv t1, t0
...
li t2, 1         # load 1
add t1, t1, t2   # perform x + 1
mv t0, sp
addi t0, t0, 16
sw t1, 0(t0)     # store the result to _T0 (*(sp + 16))
p_f_1:
mv t0, sp
addi t0, t0, 16
lw t1, 0(t0)     # load the result (from _T0)
mv t0, sp
addi t0, t0, 12
lw t0, 0(t0)     # load x (*(sp + 12))
sw t1, 0(t0)     # store the value of _T0 to the address pointed to by x
p_f_2:
li t1, 0
mv t0, sp
addi t0, t0, 8
